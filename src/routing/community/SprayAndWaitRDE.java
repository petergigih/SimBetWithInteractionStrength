 ...
 *
 */
public class SprayAndWaitRDE  implements RoutingDecisionEngine {
	...
	public SprayAndWaitRDE(Settings s)
	{
		if (s.contains(NROF_COPIES_S))this.initialNrofCopies = s.getInt(NROF_COPIES_S);
		else this.initialNrofCopies = 10;
		if (s.contains(BINARY_MODE))this.isBinary = s.getBoolean(BINARY_MODE);
		else this.isBinary = true;
	}

	public SprayAndWaitRDE(SprayAndWaitRDE proto)
	{
...
//        DTNHost dest = m.getTo();
		...
		SprayAndWaitRDE de = getOtherDecisionEngine(otherHost);
	...
	private SprayAndWaitRDE getOtherDecisionEngine(DTNHost h)
	{
		MessageRouter otherRouter = h.getRouter();
		assert otherRouter instanceof DecisionEngineRouter : "This router only works " +
				" with other routers of same type";

		return (SprayAndWaitRDE) ((DecisionEngineRouter)otherRouter).getDecisionEngine();
	}
	...
	public RoutingDecisionEngine replicate() {
		return new SprayAndWaitRDE(this);
	}
}
